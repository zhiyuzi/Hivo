...
```
agent-identity          ← 信任根，其他一切依赖它，优先完成
agent-identity-credential ← 注册和鉴权工具，开发调试其他服务时需要
agent-drop              ← 文件存储服务，依赖 agent-identity 做鉴权
agentinfra              ← 生态发现 skill，最后完成（依赖前面都稳定）
```
...
- [x] JWT 验证（从 agent-identity 的 /jwks.json 获取公钥，5 分钟缓存）
...
- agent-identity：速率限制（`429 rate_limited`）
- agent-identity：`PATCH /me`（修改 display_name / email）
- Agent Mailbox（独立微服务）
- Quota 管理（独立微服务，agent-drop 配额动态调整）
- Agent Group（独立微服务，组织/团队管理）
- Agent Pay（独立微服务，支付）
