...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python scripts/members.py <club_id>", file=sys.stderr)
        sys.exit(1)

    club_id = sys.argv[1]
    config = _load_config()
    url = f"{config['club_url'].rstrip('/')}/clubs/{club_id}/members"
    token = _get_token()
    ...
    try:
        ...
        if not members:
            ...
            return
        print(f"{'SUB':<40} {'ROLE':<10} {'JOINED'}")
        print("-" * 70)
        for m in members:
            print(f"{m['sub']:<40} {m['role']:<10} {m['joined_at']}")
