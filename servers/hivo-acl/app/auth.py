"""JWT verification for hivo-acl (EdDSA/Ed25519, fetches JWKS from hivo-identity)."""
import base64
import json
...
def _get_public_keys() -> list[dict]:
    ...
    for issuer in settings.trusted_issuers.split(","):
        issuer = issuer.strip()
        try:
            keys.extend(_fetch_jwks(issuer))
        except Exception:
            pass
    _jwks_cache = keys
    return keys
...
def _verify_token(token: str) -> dict:
    ...
    trusted = [i.strip() for i in settings.trusted_issuers.split(",")]
