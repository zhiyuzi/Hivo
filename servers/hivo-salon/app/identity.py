"""Identity service client for hivo-salon — resolve sub -> handle."""
import httpx
...
def resolve_sub(handle: str) -> str | None:
    """Resolve a handle to its sub via identity service. Returns None on failure."""
    base = _identity_base_url()
    if not base:
        return None
    try:
        r = httpx.get(f"{base}/resolve", params={"handle": handle}, timeout=TIMEOUT)
        if r.status_code == 200:
